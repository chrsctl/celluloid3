"""Wake-up, restore, epoch-prefixed replication, and compaction.
...
corrupting -- because "the epoch in the key is the fence".
"""
...
from celloid3.fragments import epoch_prefix, ltx_prefix, parse_epoch


def objects_in_epoch(bucket, cell, epoch):
    return bucket.list(epoch_prefix(cell, epoch))
...
def test_a_read_only_wake_writes_nothing(counted, embedder):
    ...
    writer = MemoryLayer(counted, cell="a", embedder=embedder, dim=256, ttl=60)
    writer.remember("the only fact")
    writer.hibernate()

    reader = MemoryLayer(counted, cell="a", embedder=embedder, ttl=60)
    reader.activate()
    counted.reset()
    ...
    reader.hibernate()
    assert not counted.inner.list(epoch_prefix("a", reader.epoch))
...
def test_group_commit_is_one_object(counted, embedder):
    ...
    mem = MemoryLayer(counted, cell="a", embedder=embedder, dim=256, ttl=60)
    mem.activate()
    counted.reset()
...
def test_flush_every_amortizes_commits(counted, embedder):
    mem = MemoryLayer(counted, cell="a", embedder=embedder, dim=256,
...
def test_a_fenced_writer_cannot_pollute_the_new_lineage(bucket, embedder):
    ...
    """
    zombie = MemoryLayer(bucket, cell="a", embedder=embedder, dim=256, ttl=0.05)
    zombie.remember("a fact from before the partition")
    time.sleep(0.08)

    successor = MemoryLayer(bucket, cell="a", embedder=embedder, ttl=60)
    ...
    third = MemoryLayer(bucket, cell="a", embedder=embedder, ttl=60)
...
def test_restore_falls_back_when_the_newest_prefix_has_no_chain(bucket, embedder):
    ...
    for key in objects_in_epoch(bucket, "a", 2):
        bucket.delete(key)
    bucket.put(epoch_prefix("a", 2) + "000000000009.tqs", b"orphan")
...
def test_compaction_collapses_a_long_chain(counted, embedder):
    ...
    for i in range(24):
        mem.remember(f"fact {i}")
    keys = counted.inner.list(epoch_prefix("a", mem.epoch))
...
def test_gc_drops_superseded_objects(bucket, embedder):
    mem = MemoryLayer(bucket, cell="a", embedder=embedder, dim=256,
                      compaction_threshold=4, ttl=60)
    for i in range(12):
        mem.remember(f"fact {i}")
    before = len(bucket.list(ltx_prefix("a")))
    assert mem.gc(keep_epochs=1) > 0
    after = bucket.list(ltx_prefix("a"))
    assert len(after) < before
    assert all(parse_epoch(k.rsplit("/", 1)[0] + "/") == mem.epoch for k in after)
    mem.hibernate()

    reader = MemoryLayer(bucket, cell="a", embedder=embedder, ttl=60)
...
def test_two_agents_never_share_an_epoch(bucket, embedder):
    epochs = []
    for _ in range(5):
        agent = MemoryLayer(bucket, cell="a", embedder=embedder, dim=256, ttl=60)
...
def test_documented_round_trip_costs(counted, embedder):
    ...
    """
    writer = MemoryLayer(counted, cell="a", embedder=embedder, dim=256, ttl=600)
    with writer.batch():
        for i in range(200):
    ...
    counted.reset()
    reader = MemoryLayer(counted, cell="a", embedder=embedder, ttl=600)
    assert (counted.gets, counted.puts, counted.lists) == (1, 0, 0)   # the config

    counted.reset()
    reader.activate()
    assert counted.lists == 2               # epoch prefixes, then that prefix
    assert counted.gets == 2                # the ownership record, then the chain
    assert counted.puts == counted.conditional_puts == 1   # the ownership claim
